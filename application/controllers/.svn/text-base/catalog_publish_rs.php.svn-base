...
class Catalog_publish_rs extends CI_Controller {
 ...
 function index(){
   if($this->session->userdata('logged_in')){
     ...
     $crud->add_action('Delete', '', 'catalog_publish_rs/doDelete','ui-icon-circle-minus');
     $crud->unset_delete();
     $crud->unset_read();

     $crud->add_action('Unpublish', '', 'catalog_publish_rs/doUnpublish','ui-icon-circle-minus');

     $crud->add_action('Order', '', 'catalog_publish_rs/doDelete','ui-icon-cart');
     ...
     $this->load->view('catalog_publish_rs_view', $data);
 ...
 function doDelete($id){
   ...
   $this->catalog_publish_rs_model->delete($dataArr, $id);
   redirect("catalog_publish_rs");
 }
...
function doInsertVariant(){
 ...
 function doUnpublish($id){
 	...
 	$this->catalog_publish_rs_model->update2Unpublish($dataArr, $id);
 	redirect("catalog_publish_rs");
 }
}
?>