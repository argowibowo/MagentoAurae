...
class Catalog_unpublish_po extends CI_Controller {
 ...
 function __construct()
 {
   ...
   $this->load->model('catalog_unpublish_po_model');
 }
 ...
 function index(){
   if($this->session->userdata('logged_in')){
     ...
     $crud->add_action('Delete', '', 'catalog_unpublish_po/doDelete','ui-icon-circle-minus');
     $crud->unset_delete();
     $crud->unset_read();

     $crud->add_action('Publish', '', 'catalog_unpublish_po/doPublish','ui-icon-circle-plus');

     $crud->add_action('Order', '', 'catalog_unpublish_po/doDelete','ui-icon-cart');
     ...
     $this->load->view('catalog_unpublish_po_view', $data);
...
function doInsertVariant(){
 ...
 function doPublish($id){
 	...
 	$this->catalog_unpublish_po_model->update2Publish($dataArr, $id);
 	redirect("catalog_unpublish_po");
 }
}
?>