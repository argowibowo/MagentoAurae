...
class Catalog_unpublish_rs extends CI_Controller {
 ...
 function __construct()
 {
   ...
   $this->load->model('catalog_unpublish_rs_model');
 }
 ...
 function index(){
   if($this->session->userdata('logged_in')){
     ...
     // dan buat join table
     $crud->set_relation('ID_CATEGORY','ms_category','NAMA',array('TIPE' => 'RS'));
     // supaya yg muncul di search awal tabel foreign dengan kondisi
     $crud->where('TIPE','RS');
     $crud->where('REC_STATUS','A');
     $crud->where('PUBLISH_STATUS','U');
     ...
     $crud->add_action('Delete', '', 'catalog_unpublish_rs/doDelete','ui-icon-circle-minus');
     $crud->unset_delete();
     $crud->unset_read();

     $crud->add_action('Publish', '', 'catalog_unpublish_rs/doPublish','ui-icon-circle-plus');

     $crud->add_action('Order', '', 'catalog_unpublish_rs/doDelete','ui-icon-cart');
     ...
     $this->load->view('catalog_unpublish_rs_view', $data);
 ...
 function doDelete($id){
   ...
   $this->catalog_unpublish_po_model->delete($dataArr, $id);
   redirect("catalog_unpublish_po");
 }
...
function doInsertVariant(){
 ...
 function doPublish($id){
 	$dataArr = array("PUBLISH_STATUS" => 'P');
 	$this->catalog_unpublish_rs_model->update2Publish($dataArr, $id);
 	redirect("catalog_unpublish_rs");
 }
}
?>